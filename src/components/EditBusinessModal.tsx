...
export const EditBusinessModal: React.FC<EditBusinessModalProps> = ({
  ...
  const [status, setStatus] = useState<BusinessStatus>('in_progress');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...record,
      sequenceNo: sequenceNo.trim() || record.sequenceNo,
      taxId: taxId.trim(),
    ...
    onShowToast(`อัปเดตข้อมูล "${companyName.trim()}" สำเร็จ`);
    onClose();
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold font-headline text-[#143c2f]">
                แก้ไขข้อมูลกิจการ ({record.sequenceNo})
              </h3>
              <p className="text-xs text-[#527d6f]">
                แก้ไขข้อมูลนิติบุคคลและสถานะการดำเนินงาน
              </p>
            </div>
          </div>
          <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                readOnly
                className="w-full h-10 px-3 mt-1 bg-[#f8faf9] text-xs font-mono font-bold rounded-xl border border-slate-200 focus:border-emerald-600 outline-none opacity-70 cursor-not-allowed"
              />
            </div>
